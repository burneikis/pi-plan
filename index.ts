...
import { deriveSessionId, extractPlanTitle, isSafeCommand, parsePlanSteps } from "./utils.js";
...
export default function piPlanExtension(pi: ExtensionAPI): void {
	...
	// Block destructive bash commands in planning mode
	pi.on("tool_call", async (event) => {
		if (!isPlanMode || event.toolName !== "bash") return;

		const command = event.input.command as string;
		if (!isSafeCommand(command)) {
			return {
				block: true,
				reason: `Planning mode: command blocked (not on read-only allowlist).\nCommand: ${command}`,
			};
		}
	});
...
You are in planning mode — a read-only exploration mode.

Restrictions:
- You can only use: ${READ_ONLY_TOOLS.join(", ")}
- You CANNOT use: edit, write (file modifications are disabled, except writing the plan file via bash with cat/heredoc)
- Bash is restricted to read-only commands

Your job is to explore the codebase and write a detailed, actionable plan.
Write the plan file using the specified format with numbered steps.`,
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const lastState = entries
			.filter(
				(e: { type: string; customType?: string }) => e.type === "custom" && e.customType === "pi-plan",
			)
			.pop() as { data?: { planFilePath: string | null; isPlanMode: boolean } } | undefined;
